# Pstack-codex maintenance

Preserve upstream engineering behavior and original MIT attribution. `upstream/pstack` is an immutable audited snapshot; never edit it in place. Runtime code lives in `plugins/pstack-codex`. Every changed or added upstream capability must have a migration-matrix row and evidence status.
