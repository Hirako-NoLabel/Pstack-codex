...
const plugin=path.join(root,'plugins/pstack-codex');
const failures=[];
...
check(manifest.name==='pstack-codex'&&manifest.skills==='./skills/','Plugin name/skills path');
const market=JSON.parse(fs.readFileSync(path.join(root,'.agents/plugins/marketplace.json'),'utf8'));
check(market.plugins.some(p=>p.name===manifest.name&&p.source.path==='./plugins/pstack-codex'),'Marketplace must resolve plugin');
