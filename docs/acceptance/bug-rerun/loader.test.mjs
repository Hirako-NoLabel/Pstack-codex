...
test('fetcher runs immediately and synchronous values stay synchronous',()=>{
 const calls=[];
 const load=createLoader(key=>{calls.push(key);return 'value';});
 assert.equal(load('alpha'),'value');
 assert.deepEqual(calls,['alpha']);
});

test('synchronous throws remain synchronous and retry works',()=>{
 let count=0;
 const load=createLoader(()=>{if(++count===1)throw new Error('sync');return 'recovered';});
 assert.throws(()=>load('alpha'),{message:'sync'});
 assert.equal(load('alpha'),'recovered');
});

test('separate loader instances do not share pending work',async()=>{
 const left=createLoader(async()=> 'left');
 const right=createLoader(async()=> 'right');
 assert.deepEqual(await Promise.all([left('alpha'),right('alpha')]),['left','right']);
});
