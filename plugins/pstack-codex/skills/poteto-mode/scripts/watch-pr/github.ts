...
export function orderStack(
  ...
  let current = start;
  const parentSeen = new Set<T.PrNumber>();
  while (byHead.has(current.baseRefName)) {
    if (parentSeen.has(current.number)) throw new WatcherQueryError({ kind: "missing-key", retryable: true, detail: "cycle in PR base chain" });
    parentSeen.add(current.number);
