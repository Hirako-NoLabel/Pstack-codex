...
3. Explore in subagents with `the available host subagent tool` and role routing from the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
The program runs `<installed-plugin-root>/skills/poteto-mode/playbooks/<execution playbook>.md`. <Who merges, and which PR ids are the operator's items that stop at merge-ready.>
...
- [ ] On the operator's go, save the recorded goal using an available goal tool or durable checkpoint, with this exact text. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>"
- [ ] Read these installed workflow files at program start. Re-read them at every tick.
  - [ ] `read <installed-plugin-root>/skills/poteto-mode/playbooks/<execution playbook>.md`
  - [ ] `read <installed-plugin-root>/skills/swarm/SKILL.md`
  - [ ] `read <available-control-tool-instructions>`
  - [ ] `read <installed-plugin-root>/skills/poteto-mode/playbooks/opening-a-pr.md`
  - [ ] `read <installed-plugin-root>/skills/<each other leaf skill the program uses>`
- [ ] Arm the 30-minute audit tick. In a local session, a supported host heartbeat or event wait. The same capability check applies in cloud environments. Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the installed workflow and the recorded goal. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then post a status message to the operator in chat, whether or not anything changed, with the queue table of PR, owner, state, and head SHA, the verdicts since the last tick, what merged, open operator gates, and blockers."
...
- [ ] Run the runtime diff cleanup procedure before each commit and `/no-comments` before review.
- [ ] Triage every Bugbot and security-reviewer comment per `../references/bugbot-triage.md`.
- [ ] Rebase onto current trunk before babysit and again before the merge-ready report.

### Verdict and merge, for every PR

- [ ] At the merge-ready head SHA, run the swarm per `<installed-plugin-root>/skills/swarm/SKILL.md`. One gates lane. The ten live lanes from the PR's **Verify, live** block. The perf lane from its **Verify, perf** block. One audit lane that reads the diff and the receipts and distrusts the PR body.
- [ ] Clean only when every lane is `PASS`. Findings go back to the owner. A new head gets a fresh swarm and a fresh verdict.
- [ ] <The merge or append rule from the execution playbook, with the patch-id rule from `playbooks/shipping.md`.>

### Boot recipe, for every live lane

Each live lane uses an isolated checkout at the PR head. Use separate machines only if the host provides them; otherwise disclose the shared environment and run independent lanes in bounded waves. Drive through the available UI or CLI control tool.

- [ ] `git fetch origin <head-branch> && git checkout <head SHA>`.
- [ ] <Start the backend and the surface. Wait for ready.>
- [ ] <Deliver input only through the control skill's commands. Name the read-only diagnostics.>
- [ ] Save every screenshot to `<artifact-directory>/swarm-<pr-id>/worker-<n>/<slug>.png` (resolve a writable persistent artifact directory on the current platform) and return the paths with the report.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten independent scenario lanes at the PR head, per the boot recipe.
...
<Docs to read before editing. Which PRs get `<installed-plugin-root>/skills/how/SKILL.md` and `<installed-plugin-root>/skills/interrogate/SKILL.md`. The trail per `<installed-plugin-root>/skills/show-me-your-work/SKILL.md`.>
````
