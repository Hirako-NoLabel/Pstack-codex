...
Continue until done; use an available authorized host goal or heartbeat for wakeups. if you're truly stuck after a few hours, stop and write up why.
```

Walk through what each line buys you:

- "im going to bed" is a session override. The agent continues authorized work without routine check-ins; existing safety, permission and runtime limits still apply.
...
- Background wakeups require a real host capability. Use an explicit goal when supported, or a user-authorized native automation/heartbeat when available. A skill alone cannot keep a closed app running or bypass limits. If no persistent scheduler is available, checkpoint the run and report that automatic resumption is unavailable. See the [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md).
...
Before the skill hands back its summary, it spawns a reviewer on a different available model family, or an independent role when model overrides are unavailable to read the trail and the transcript, and the reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
