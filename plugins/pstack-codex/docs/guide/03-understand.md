...
[`$recall`](../../skills/recall/SKILL.md) uses accessible recent chats, Git history, AGENTS.md, handoff files and the shared record (issues, prior fixes, errors still firing) and hands back a brief on where things stand and what's next. Use it when you're returning to a topic cold. If you want to resume one specific chat, that's the Session pickup playbook below, not `$recall`.
