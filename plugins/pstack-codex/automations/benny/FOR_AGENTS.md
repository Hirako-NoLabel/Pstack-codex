...
- i want a verified project-scoped PStack installation for shared dependencies such as `how`, `why`, `tdd`, `unslop`, and the required principle skills. do not invent a host settings file.
...
the human enters setup by pointing the OpenAI host at this file. do not look for or invoke a discovered benny slash skill.
...
Use the copied setup file's host capability checks, creation/update flow and thread-safety test. Use only documented native host automation tools. Native Slack event triggering is not assumed; unsupported triggers remain recorded, and polling requires an explicit configured adapter. Live integrations remain Untested until exercised.

The live triage prompt reads `.pstack/automations/benny/skills/triage-issue-reports/SKILL.md`; repro reads `.pstack/automations/benny/skills/reproduce-and-fix-issues/SKILL.md`. Use committed repository-relative paths, never a plugin-cache path. Keep existing automations distinct from creation and do not create duplicates.
