...
You don't describe your style, because [`$automate-me`](../../skills/automate-me/SKILL.md) derives it from accessible evidence. It uses available task history, AGENTS.md, project rules and Git history for repeated preferences, in how you like replies, delegation, verification, code, prose, and process, then asks you which patterns are really you. It drafts `.agents/skills/<your-name>-mode/SKILL.md` through the available skill-authoring workflow, runs the draft through [`$unslop`](../../skills/unslop/SKILL.md), and opens a PR from a worktree so you review it like any other change.
...
[`$reflect`](../../skills/reflect/SKILL.md) sends accessible session evidence to three independent reviewers (queued if concurrency is limited), then a synthesizer sorts the proposals into `Accepted`, `Rejected`, and `Backlog` and waits for your approval before any skill changes. Approve a proposal only if it would change a future decision. One weird session is an anecdote, not a rule.
