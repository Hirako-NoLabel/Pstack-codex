...
[`typescript-best-practices`](../../skills/typescript-best-practices/SKILL.md) should be selected when the agent touches a `.ts` or `.tsx` file; invoke `$typescript-best-practices` explicitly if the host does not select it and turns the type-system principles into concrete rules: discriminated unions, `unknown` at boundaries, exhaustive variants, schema-derived types.

## Clean before you commit

The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs the available code-cleanup pass on the diff before each commit and applies [`$unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. `deslop` was an external upstream dependency and is not bundled here. Use an available cleanup skill, or ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
