...
- Set `NOTES_DATA_DIR` to a fresh directory under the platform temporary directory (for example Node `fs.mkdtempSync(path.join(os.tmpdir(), 'notes-verify-'))`) so concurrent runs do not share state. Set the environment variable through the current shell's syntax.
