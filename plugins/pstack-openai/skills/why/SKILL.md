...
Before spawning investigators, discover enabled tools and connectors from the active host tool catalog. Use tool search when exposed. Do not inspect a presumed Cursor `mcps/` directory. Treat reference tool names as examples; discover each actual tool schema before calling it.
...
Check whether git history and authenticated `gh` are available. If either is absent, report the precise coverage gap and use only accessible repository or supplied PR evidence. For the other six, classify using the MCP name, server instructions, tool names, and resource descriptors. If an MCP could fit more than one category, choose the one matching its primary evidence. Record ambiguous cases in the coverage map.

Aim for a complete **coverage map**, not a minimal one. Document the null, don't skip the search.

Schedule all matching investigators within the host concurrency limit. Don't ask one agent to cover multiple MCPs.

Subagent config (each):
- Use the native subagent API with the relevant investigator or synthesizer prompt.
- `model`: your configured why-investigators model (default `inherit-parent`)
- Read-only task contract: no file or external-state writes. Connector access is host-dependent; do not enable write permissions merely because Cursor formerly required agent mode for MCP access.
...
1. **Source control investigator**. Git history, `gh` for PRs, code comments, tests. Use whenever available. Missing git or PR access is a documented gap. Best at surfacing *implementation-time rationale captured during review*.
...
- Use the native subagent API with the relevant prompt.
- `model`: your configured why-synthesizer model (default `inherit-parent`)
- Read-only task contract. Spot-check citations through accessible connectors without modifying state.
