...
it("rejects a cyclic stack instead of hanging during parent traversal", () => {
  expect(() => orderStack(context, [
    {number:context.number,headRefName:"a",baseRefName:"b"},
    {number:parsePrNumber(43),headRefName:"b",baseRefName:"a"},
  ])).toThrow("cycle in PR base chain");
});
