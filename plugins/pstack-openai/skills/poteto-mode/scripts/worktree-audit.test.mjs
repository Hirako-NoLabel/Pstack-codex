...
test('real git worktree audit is read-only and handles a space and Unicode path',()=>{
 ...
 try{
  ...
  assert.equal(report.rows[1].path.replaceAll('\\','/'),wt.replaceAll('\\','/'));assert.equal(report.rows[1].bucket,'hold-untracked');assert.ok(report.rows[1].sizeBytes>0);assert.equal(report.rows[1].ageDays,0);assert.equal(report.rows[1].remote,'no-remote');assert.equal(report.prLookup,'not-requested');
