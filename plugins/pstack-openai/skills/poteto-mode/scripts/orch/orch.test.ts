...
describe("Store", () => {
  ...
  it("persists a provider-derived frontier with remote head SHA and generation", async () => {
    const seen: unknown[]=[];
    const { store } = await initializedStore({ frontierReader(repo, pin) {
      seen.push({ repo, pin });
      return [{pr:10,branches:"stack/merged",sha:"a".repeat(40),state:"MERGED"},
        {pr:11,branches:"stack/open",sha:"b".repeat(40),state:"OPEN"}];
    }});
    expect(await store.frontier.set({repo:".",prs:[10,11]})).toMatchObject({generation:1,lowestUnmerged:11});
    expect((await store.frontier.set({repo:".",prs:[10,11]})).generation).toBe(2);
    expect((await store.frontier.show()).prs[1].sha).toBe("b".repeat(40));
    expect(seen).toHaveLength(2);
    await expect(store.frontier.set({repo:".",prs:[10,10]})).rejects.toThrow("duplicates");
  });
