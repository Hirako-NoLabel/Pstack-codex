...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned and continued only through actual host subagent tools, never simulated with user-owned tasks. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting and concurrency obey the actual host limits; flatten tracks if nested spawning is unavailable). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Use available subagent isolation. This machine is required for local control tools, authorized local history, simulators, IDE state, or local authentication. Remote workers are optional and cannot assume access to a local store; relay their required context explicitly. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Use an independent verifier, preferably a different configured model when supported; record absent model diversity.
...
Create `<target-repository>/.pstack/orchestrate/<project-slug>/`, or an explicitly chosen writable persistent store. Do not assume the host exposes an agent-store path. Every file has exactly one writer. Owners publish facts, readers aggregate at read time. Resolve `scripts/orch/orch.ts` relative to this installed poteto-mode skill and invoke `bun <absolute-script-path> --store <absolute-store-path>`. This is written below as `orch`; its canonical plain TSV and JSON stay readable without the CLI.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, proposed default for reversible choices only; silence never approves an authorization gate).
...
A sub-coordinator brief adds its track boundary and unit list, its actual host spawn budget and environment constraints, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (use an actually available native event wait or explicitly requested heartbeat), and before a human report. Without a scheduler, continue in-session and preserve a resume checkpoint. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` with `orch frontier set --repo <repo> --prs <bottom-to-top-list>` after every merge and stack mutation. The CLI reads GitHub base/head refs and rejects cycles, branching, missing PRs, and order drift. A frozen explicit list preserves merged items across retargets. Record ordered PRs, branches, current remote SHAs, generation, and the lowest unmerged PR.
- Exactly one stacker per stack changes branch topology, serialized within its stack. Record the holder in standing orders. Use an isolated checkout; choose local or remote placement from actual host resources. Graphite is not required.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the available host agent status tool. Transcript mtime is not liveness.
...
- After a host restart, inspect actual agent status; never assume any worker survived. Re-read the standing orders and `units.tsv`, recompute the frontier, recover durable work by PR and branch, and reconnect workers only if the host confirms they exist, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
