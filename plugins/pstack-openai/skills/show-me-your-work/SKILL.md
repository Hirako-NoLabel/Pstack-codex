...
Use `node scripts/log.mjs <logfile> <phase> <decision> <why> <evidence> <result>` on Windows, macOS, or Linux. `scripts/log.sh` is a POSIX wrapper for the same implementation. It stamps `ts`, writes the header on first use, strips stray tabs/newlines, and prefixes any cell starting with `=`, `+`, `-`, or `@` with a single quote. A bare `printf` appending a row works too, but mind those same bytes if cells come from generated or user-supplied text.
...
At the end of the run, before handing back, check the log told the truth. Read this run's accessible native task history, supplied transcript, or labeled evidence digest. Never mine global or unrelated project history. If full tool history is unavailable, disclose that audit coverage limit. Walk the log against what actually happened:

- Every row maps to a real action. Append correction rows invalidating invented or aspirational entries; never erase history.
- Each row's evidence resolves and shows what the row claims.
- A fork, pivot, or abandoned approach that shaped the work but isn't logged is a gap. Add it.
- Identify padding in a superseding correction rather than deleting historical rows.

Fix the log, not the story. If the work diverged from what a row claims, the row is wrong.

## Cross-model review of the trail

Before handing back, use an independent native reviewer. Prefer a different model family only if verified available and configured. Otherwise record the same-model limitation; do not fabricate cross-model review. If no subagent tool exists, report independent review unavailable. Self-review is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
