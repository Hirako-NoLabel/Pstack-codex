...
1. Spawn a native subagent with the role prompt in `../../agents/comment-sicko.md`. Pass the exact scope and the role text; do not assume the role name is registered by the host.
...
5. Constraint comments say `do not remove`, `do not change wording`, or `talk to X before changing`. Leave keeps about things we cannot change. Offer the cheapest in-scope type, runtime, test, or CI lint. Wait for interactive approval. Unattended and eval require caller pre-approval. If approved, encode then delete. If approval is absent, retain an exception-protected constraint comment, report the constraint open, and sketch out-of-scope work. Never treat silence as approval to weaken a safety constraint.
