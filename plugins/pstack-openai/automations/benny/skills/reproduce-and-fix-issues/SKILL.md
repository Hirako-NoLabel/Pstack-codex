...
This is a dormant operational source, not a discoverable plugin skill. Read [protocol helper](../../scripts/protocol.mjs) when implementing an adapter. Normalize the trigger to `{source_channel_id, message_ts, thread_ts}` before applying these gates. The adapter must persist idempotence claims atomically; the pure helper does not supply distributed storage. Live integrations remain Untested.
...
1. Require `trigger.source_channel_id` to equal the configured source channel.
2. Set `SOURCE_THREAD_TS` to `trigger.thread_ts` when present. Otherwise use `trigger.message_ts`.
...
Prefer configured available host Slack actions. Use `BENNY_SLACK_BOT_TOKEN` only when the user configured it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.
